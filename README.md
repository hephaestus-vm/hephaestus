...
> [!NOTE]
> Hephaestus currently publishes prereleases only, so the installer requires an
> explicit `--version`. This prevents an installation from silently moving to a
> newer alpha or beta. After the first stable release, omitting `--version` will
> install the latest stable version; specifying a version will continue to
> provide a reproducible installation.
