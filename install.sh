...
# - While Hephaestus only publishes prereleases, --version is required so the
#   installer never selects an alpha or beta release implicitly.
# - After the first stable release, an omitted version should resolve to the
#   latest stable release.
# - --version remains supported for reproducible installations.
#
# TODO(stable): implement latest-stable resolution before making --version
# optional.
set -eu
...
[ -n "$VERSION" ] \
    || fail "--version is required while Hephaestus only publishes prereleases;" \
        "choose a tag from $REPOSITORY/releases"
printf '%s\n' "$VERSION" \
